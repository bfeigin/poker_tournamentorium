module PokerCalculator
  class PokerHand
    ...
    def initialize(cards = [])
      ...
      when Array
        cards.map do |card|
          if card.is_a? CardHolder
            card
          else
            CardHolder.new(card.to_s)
          end
        end
      when String
        cards.scan(/\S{2}/).map { |str| CardHolder.new(str) }
      else
        cards
      end
    ...
    def straight_flush?
      if (md = (/.(.)(.)(?: 1.\2){4}/.match(delta_transform(true))))
        high_card = CardHolder::face_value(md[1])
    ...
    def four_of_a_kind?
      if (md = (by_face =~ /(.). \1. \1. \1./))
        # get kicker
        result = [8, CardHolder::face_value(md[1])]
        result << CardHolder::face_value($1) if (md.pre_match + md.post_match).match(/(\S)/)
    ...
    def full_house?
      if (md = (by_face =~ /(.). \1. \1. (.*)(.). \3./))
        ...
        [
          [7, CardHolder::face_value(md[1]), CardHolder::face_value(md[3])],
          arranged_hand
        ]
        ...
        [
          [7, CardHolder::face_value(md[5]), CardHolder::face_value(md[2])],
          arranged_hand
        ]
      else
        false
    ...
    def flush?
      if (md = (by_suit =~ /(.)(.) (.)\2 (.)\2 (.)\2 (.)\2/))
        [
          [
            6,
            CardHolder::face_value(md[1]),
            *(md[3..6].map { |f| CardHolder::face_value(f) })
          ],
          arrange_hand(md)
        ]
      else
    ...
    def three_of_a_kind?
      if (md = (by_face =~ /(.). \1. \1./))
        ...
        if matches
          result = [4, CardHolder::face_value(md[1])]
          matches = arranged_hand.match(/(?:\S\S ){3}(\S)/)
          result << CardHolder::face_value($1) if matches
          matches = arranged_hand.match(/(?:\S\S ){3}(\S)\S (\S)/)
          result << CardHolder::face_value($2) if matches
    ...
    def two_pair?
      ...
      if (md = (by_face =~ /(.). \1.(.*?) (.). \3./))
        ...
        if matches
          result = []
          result << 3
          result << CardHolder::face_value(md[1])    # face value of the first pair
          result << CardHolder::face_value(md[3])    # face value of the second pair
          matches = arranged_hand.match(/(?:\S\S ){4}(\S)/)
          result << CardHolder::face_value($1) if matches    # face value of the kicker
        return [result, arranged_hand]
        end
      end
      false
    ...
    # Returns an array of `CardHolder` objects that make up the `PokerHand`.
    def to_a
      @hand
    end
    alias :to_ary :to_a
    ...
    def << new_cards
      if new_cards.is_a?(CardHolder) || new_cards.is_a?(String)
        new_cards = [new_cards]
      end
    ...
    #     hand.delete("Jd")           # => #<CardHolder:0x5d0674 @value=23, @face=10, @suit=1>
    #     hand.just_cards             # => "5d"
    def delete card
      @hand.delete(CardHolder.new(card))
    end
    ...
    def match? expression
      ...
      if expression =~ /^(.)(.)(s|o|)(\+|)$/
        face1 = CardHolder.face_value($1)
        face2 = CardHolder.face_value($2)
    ...
    def +(other)
      cards = @hand.map { |card| CardHolder.new(card) }
      case other
      when String
        cards << CardHolder.new(other)
      when CardHolder
        cards << other
      when PokerHand
        cards += other.hand
    ...
    def delta_transform(use_suit = false)
      aces = @hand.select { |c| c.face == CardHolder::face_value('A') }
      aces.map! { |c| CardHolder.new(1,c.suit) }
